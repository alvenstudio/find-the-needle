  ...
  build(ringRadius: number, options: { rebirthUnlocked: boolean; hasNextTier: boolean }): void {
    ...
    const layout: { id: InteractionId; model: string; label: string; hint: string; angle: number; radius: number; enabled: boolean }[] = [
      ...
      },
      {
        ...
        model: 'upgrade_kiosk',
        label: 'Улучшения',
        hint: 'Потратить монеты на снаряжение',
        angle: Math.PI * 1.72,
        radius: 3.4,
        enabled: true,
      },
      {
        id: 'tools',
        model: 'storage_silo',
        label: 'Сарай с инструментом',
        hint: 'Купить и взять инструмент',
        angle: Math.PI * 1.28,
        radius: 3.4,
        enabled: true,
      },
      ...
      },
      {
        ...
        label: 'На покой',
        hint: 'Обменять всё на постоянные бонусы',
        angle: Math.PI * 0.72,
        radius: 3.6,
      ...
      },
      {
        ...
        model: 'tier_gate',
        label: 'Другой стог',
        hint: 'Отправиться к стогу побольше',
        angle: Math.PI * 1.0,
        radius: 4.0,
